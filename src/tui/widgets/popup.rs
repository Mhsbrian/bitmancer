...
fn render_nickname_popup(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let border_style = theme::border(app.focus_area == FocusArea::InputBox);
...
fn render_connection_popup(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let border_style = match &app.phase {
        TuiPhase::Error(_) => Style::default().fg(theme::FAULT),
        _ => Style::default().fg(theme::CHROME),
    };

    let popup_block = Block::default()
        .title(theme::panel_title(match &app.phase {
            TuiPhase::Error(_) => "link failed",
            _ => "link",
        }))
        .title_alignment(Alignment::Center)
        .borders(Borders::ALL)
    ...
    // Status section. A rotating braille frame instead of an hourglass: it says
    // "still working" without blinking for attention.
    let status_text = match &app.phase {
        TuiPhase::Error(_) => "△  no link".to_string(),
        TuiPhase::Connecting => format!("{}  acquiring link", theme::spinner(app.tick)),
        _ => "◈  linked".to_string(),
    };
    let status_para = Paragraph::new(status_text)
        .style(match &app.phase {
            TuiPhase::Error(_) => Style::default().fg(theme::FAULT).bold(),
            TuiPhase::Connecting => Style::default().fg(theme::LIVE),
            _ => Style::default().fg(theme::MINE).bold(),
        })
        .alignment(Alignment::Center);
    ...
    let instructions_text = match &app.phase {
        TuiPhase::Error(_) => "r retry   esc dismiss   ctrl+c quit",
        _ => "esc dismiss - the client works offline",
    };
    let instructions_para = Paragraph::new(instructions_text)
        .style(Style::default().fg(theme::DIM))
        .alignment(Alignment::Center);
